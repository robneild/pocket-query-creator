...
import org.pquery.webdriver.CreateAsync;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		async = new CreateAsync(true, getApplicationContext(), queryStore,
			...
			protected void onProgressUpdate(ProgressInfo... values) {
				...
				//	runningNotification = new Notification(R.drawable.status_bar,
				//			getText(R.string.local_service_started), System.currentTimeMillis());

				//	runningNotification.setLatestEventInfo(getApplicationContext(),
				//			getText(R.string.local_service_started), "", getPendingIntent());

					// RemoteViews contentView = new
					// RemoteViews(getPackageName(),
					// R.layout.custom_notification);
					// contentView.setImageViewResource(R.id.image,
					// R.drawable.status_bar);
					// runningNotification.contentView = contentView;
				//}

				//runningNotification.contentView.setTextViewText(R.id.title, values[0].message);
				//runningNotification.contentView.setProgressBar(R.id.notification_progress_bar, 100, values[0].percent,
				//		false);

				//mNM.notify(NOTIFICATION_IN_PROGRESS, runningNotification);
			}
