...
public class SuccessMessageParserTest extends AndroidTestCase {
    ...
    public void testExtractDownloadLink() throws FailurePermanentException {
        assertEquals("dd2e3f12-2c04-4090-bd1b-4e601f521fe5", new SuccessMessageParser(GOOD, getContext().getResources()).extractDownloadGuid());
    }
    ...
    public void testExtractDownloadLinkBad() throws FailurePermanentException {
        SuccessMessageParser p = new SuccessMessageParser("<p class=\"Success\">Thanks! Your pocket query has been saved and currently results in 10 caches. You can <a href=\"http://www.geocaching.com/seek/nearest.aspx?pq=dd2e3f12-2c04-4090-bd1b-4e601f</p>", getContext().getResources());
        try {
    ...
    public void testExtractNumberPQ() throws FailurePermanentException {
        assertEquals(10, new SuccessMessageParser(GOOD, getContext().getResources()).extractNumberPQ());
    }

    public void testExtractNumberPQBad() throws FailurePermanentException {
        SuccessMessageParser p = new SuccessMessageParser("<p class=\"Success\">Thanks! Your pocket query has been saved and currently. You can <a href=\"http://www.geocaching.com/seek/nearest.aspx?pq=dd2e3f12-2c04-4090-bd1b-4e601f521fe5\" title=\"Preview the Search\">preview the search</a> on the nearest cache page.</p>", getContext().getResources());
        try {
